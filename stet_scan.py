...
async def scanpool(taskarray):
    """
    Функция scanpool() изспользуется для создания асинхронных задач для пула на сканирование узлов локальной сети.

    Args
    ----
        taskarray - type:list - список узлов сети, которые должны войти в пул

    Return
    ------
        rresultdict - type:dictionary - словарь с информацией о сканировании узлов. Имеет туже мощность, что и список taskarray. Ключи - IP-адреса

    Note
    ----
        Возможна задержка сканирования при использовании устаревших параметров для утилиты nmap.
        Смотрите документацию https://nmap.org/book/man.html
    """
    tasksarray = {}
    resultdict = {}
...
def StartScanning(
    targetarg, log, ChClient, idScan_, insertDate_
) -> None:
    """
    Функция StartScanning() используется для начала сканирования локлаьной сети. В ней же производится коннекция с БД и запись данных в соответсвующие таблицы.
    
    Args
    ----
        targetarg - type:list - список активных адресов сети
        log - type:LoggerObject - объект кастомного логгера
        ChClient - type:connector - коннектор к БД
        idScan_ - type:integer - номер текущего сканирования относительно записей в БД
        insertDate_ - type:integer - время начала сканирования в формате timestamp
        ...
        if scannedHosts < len(targetarg) + 1:
            ...
            if result != False:
                for host in targetarray:
                    ...
                    AnalyseScanResults(result[host], log, idScan_, insertDate_, atomicInsert['aboutHost'], ChClient, host)
...
def main():
    """
    Функция main() осуществляет обработку аргументов КС, 
    обрабатывает диапозон сканирования, 
    создает очередь узлов на сканирование и делает запись о начале сканирования в БД.
    ...
    if len(targetarg) != 0:
        ...
        targets = DiscoverHosts(targets, scantype, scanmode)
    ...
    #print(targets)
    #print("\nTargets list ^^\n")
    StartScanning(targets, log, ChClient, idScan, insertDate)
